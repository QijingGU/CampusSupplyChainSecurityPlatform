...
function savePassword() {
  ...
  ElMessage.success('演示环境未连接修改密码接口，表单已校验通过')
  pwdForm.current = ''
  pwdForm.next = ''
  pwdForm.confirm = ''
}
...
<template>
  <div class="settings-panel">
    ...
    <el-card shadow="never" class="block">
      ...
      <el-form label-width="100px" style="max-width: 420px">
        <el-form-item label="当前密码">
          <el-input v-model="pwdForm.current" type="password" show-password placeholder="演示可留空" />
        </el-form-item>
    ...
    <el-card shadow="never" class="block">
      ...
      <p class="hint">
        新版工作台以悬浮式输入、提示词胶囊与瀑布流为主；此处选项仅作演示存储，后续可接入为「胶囊/发现流」开关。
      </p>

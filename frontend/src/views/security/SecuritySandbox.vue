...
function patchAnalysisAfterCapture(row: QuarantineItem) {
  ...
  if (!a) {
    ...
    const base: QuarantineAnalysis = {
      ...
      daily_counts: counts,
      insights: [
        `【动态沙箱】${formatTime(row.modified_at)} 检出可疑脚本样本 ${row.saved_as}（演示数据）。`,
  ...
  ins.unshift(
    `【动态沙箱】${formatTime(row.modified_at)} 捕获可疑对象 ${row.saved_as}（演示数据，未写入业务存储）。`
  )
...
function buildCaptureReport(row: QuarantineItem, iso: string) {
  ...
  return {
    ...
    sections: [
      {
        title: '执行摘要',
        body: `沙箱在 ${formatTime(iso)} 将样本标为「中风险 · PHP 可疑脚本」。本段为演示流程生成，用于展示报告结构；生产环境须以真实沙箱遥测与杀毒引擎结论为准。`,
      },
      {
        title: '静态分析',
...
function detailNarrative(row: QuarantineItem): string {
  ...
  if (row.local_only) {
    lines.push('【演示】由「运行分析任务」流程注入的模拟样本，未写入服务端上传目录。')
  }
  if (r === 'high') {
    lines.push(
...
async function runSyncSequence() {
  ...
  const run = async () => {
    ...
    pushCaptureLog('【告警】命中 PHP 可疑脚本特征（演示任务注入）')
    await sleep(280)
...
function openFile(row: QuarantineItem) {
  if (row.local_only || !row.url?.trim()) {
    ElMessage.info('该条为演示样本，无可下载实体链接。')
    return
  }
  ...
  <div class="sandbox-page">
    ...
    <section class="sb-console" aria-label="沙箱运行概览">
      <div class="sb-runtime-grid">
        ...
        <div class="sb-runtime-card">
          ...
          <div class="sb-runtime-card__value mono-tiny">NAT · 外联模拟</div>
            ...
            >
              <el-table-column prop="saved_as" label="样本文件" min-width="240">
                <template #default="{ row }">
                  ...
                  <span v-if="row.local_only" class="demo-badge">演示</span>
                </template>
              </el-table-column>
    ...
    >
      <template v-if="lastCaptureReport">
        ...
        <p class="report-foot">
          本报告由演示任务生成，仅供界面演练；生产环境须对接真实沙箱遥测与杀毒引擎结论。
        </p>
        <div class="drawer-actions">

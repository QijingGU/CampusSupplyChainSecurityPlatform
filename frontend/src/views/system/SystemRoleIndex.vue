...
/** 列表行：含演示扩展行；campusRole 仅内置五角色有值，用于菜单权限演示 */
interface RoleRow {
  roleId: number
  roleName: string
  roleCode: string
...
function campusDescription(code: RoleType): string {
  const m: Record<RoleType, string> = {
    ...
    warehouse_procurement: '入库出库、库存、配送协同与仓储大屏',
...
function buildAllRows(): RoleRow[] {
  ...
  for (let i = 6; i <= 100; i++) {
    ...
    extra.push({
      ...
      description: `${t.description}（演示数据 #${i}）`,
      enabled: (i + gen) % 7 !== 0,
...
function savePerm() {
  ElMessage.success('已保存（演示数据，实际权限以登录角色为准）')
...
function saveAdd() {
  ...
  tableData.value.unshift({
    ...
    description: addForm.description.trim() || '新建角色（演示）',
    enabled: true,
    createTime: randomTime(nextId * 31),
  })
  page.value = 1
  ElMessage.success('已添加（仅前端演示）')
  addVisible.value = false
}

function onDelete(row: RoleRow) {
  if (row.campusRole) {
    ElMessageBox.alert('内置校园角色不可删除，仅演示扩展行可移除。', '提示', { type: 'info' })
    return
  }
  ElMessageBox.confirm(`确定删除角色「${row.roleName}」吗？（演示）`, '删除确认', {
    type: 'warning',
  })
    .then(() => {

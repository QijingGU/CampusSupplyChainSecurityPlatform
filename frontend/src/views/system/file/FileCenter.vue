...
function onBatchDelete() {
  ...
  ElMessageBox.confirm(`确定删除选中的 ${selected.value.length} 个文件？（演示）`, '提示', { type: 'warning' })
    .then(() => {
...
function downloadMock(row: FileCenterRow) {
  ElMessage.success(`已开始下载：${row.name}（演示）`)
}
...
<template>
  <div ref="fullscreenRef" class="sys-page art-full-height">
    ...
    <el-dialog v-model="uploadVisible" title="上传文件（演示）" width="440px" destroy-on-close>

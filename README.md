...
- These flows were validated on 2026-04-05 by uploading `tmp/codex-note.txt` (pass, released to the public path) and `tmp/codex-webshell.php` (quarantined, visible in the sandbox, and reflected as a real incident on the situation page).
  ...
  On 2026-04-07 the admin-only popup flow was also replayed live: two quarantined PHP uploads generated high-risk events `#77` and `#76`, `system_admin` received the first popup and the queued second popup after about `10.67s`, while `logistics_admin` did not receive any popup during a 12-second wait after a later high-risk upload event.

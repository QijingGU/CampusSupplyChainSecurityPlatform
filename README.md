...
- These flows were validated on 2026-04-05 by uploading `tmp/codex-note.txt` (pass, released to the public path) and `tmp/codex-webshell.php` (quarantined, visible in the sandbox, and reflected as a real incident on the situation page).
  ...
  On 2026-04-07 the admin-only popup flow was replayed live again with real upload incidents: `system_admin` saw the next queued popup about `10.327s` after closing the first one, `今日不再弹出` suppressed the rest of that browser-day, `跳转 IDS 页面` landed on `/security/ids?event=87&report=1`, and `logistics_admin` still received no popup during a 15-second watch window.

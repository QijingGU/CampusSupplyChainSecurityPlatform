...
for un, rn in [
    ...
    if not db.query(User).filter(User.username == un).first():
        db.add(
            User(
                username=un,
                hashed_password=get_password_hash("123456"),
...
# 完整业务链路演示：POCHAIN-* 采购单、溯源、配送、出入库、多批次库存（首次执行写入，见 demo_dataset_cohesive）
if apply_cohesive_demo(db, force=False):
    print("[init_db] 已写入 POCHAIN 完整演示数据（订单/溯源/库存/审计）。重建请执行: python -m app.demo_dataset_cohesive --force")

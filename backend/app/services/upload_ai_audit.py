"""Upload audit helpers for public file intake."""
...
def _build_guardrail_result(
    *,
    ...
    if verdict == AUDIT_VERDICT_PASS:
        ...
        actions = ["允许进入公开目录并保留审计留痕。", "如需更高精度，可启用 AI 审计增强。"]
...
def _build_prompt(
    *,
    ...
    return f"""你是校园供应链安全平台的上传审计助手。请根据文件元数据、静态特征和内容摘录，对文件给出三类结论之一：
- pass: 可以进入公开上传目录
...
def audit_upload_payload(
    *,
    ...
    if clean_trusted_format:
        ...
        summary = f"{file_name} matches the expected {label} file structure and no dangerous indicators were found."
        evidence = [
            f"File signature matches the declared {label} format.",
            "No script execution, download, macro, or executable payload indicators were detected.",
        ]
        actions = [
            "Allow the file into the accepted upload directory and keep the audit trail.",
            "If needed, review the stored hash and audit report in the security center.",
        ]

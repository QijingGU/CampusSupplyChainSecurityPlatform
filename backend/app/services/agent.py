...
from .agent_tools import process_chat_agent, process_chat_admin, process_chat_warehouse
...
def process_chat(db: Session, message: str, applicant_id: int | None = None, history: list | None = None, user_role: str = "") -> dict:
    ...
    if settings.LLM_BASE_URL and settings.LLM_PROVIDER in ("deepseek", "openai"):
        result = process_chat_agent(db, message, applicant_id, history, user_role=role)
        if result.get("reply"):
            return result
        if role != "counselor_teacher":

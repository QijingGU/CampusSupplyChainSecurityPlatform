"""Public upload, quarantine, and sandbox analysis endpoints."""
...
from ..services.ids_ingestion import REAL_EVENT_ORIGIN, SOURCE_TRANSITIONAL_LOCAL, apply_source_metadata, build_correlation_key, build_event_fingerprint
...
ACCEPTED_DIR = ROOT_DIR / "uploads" / "accepted"
QUARANTINE_DIR = ROOT_DIR / "quarantine_uploads"
REPORT_DIR = ROOT_DIR / "upload_reports"
...
def _actor_fields(current_user: User | None) -> tuple[int | None, str, str]:
    if current_user is None:
        return None, "anonymous_upload", "public"
    return current_user.id, (current_user.real_name or current_user.username or "system_admin")[:64], (current_user.role or "system_admin")[:64]


def _write_ids_log(db: Session, *, current_user: User | None, action: str, target_type: str, target_id: str, detail: str) -> None:
...
def _sync_ids_event_from_report(db: Session, request: Request, report: dict[str, Any]) -> int:
    ...
    apply_source_metadata(event, event_origin=REAL_EVENT_ORIGIN, source_classification=SOURCE_TRANSITIONAL_LOCAL, detector_family="upload_audit", detector_name="upload_ai_gate", source_rule_id="upload_ai_gate", source_rule_name="upload_ai_gate", source_version=str(audit.get("provider") or audit.get("engine") or "upload_audit"), source_freshness="current", occurred_at=created_at, event_fingerprint=fingerprint, correlation_key=correlation)
    db.flush()
...
@router.post("")
async def public_upload(request: Request, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        _write_ids_log(db, current_user=None, action="ids_upload_rejected", target_type="public_upload", target_id=file_name[:64], detail=f"Upload audit failed before storage: {exc}")
        db.commit()
    ...
    if quarantined:
        ...
        _write_ids_log(db, current_user=None, action="ids_upload_quarantine", target_type="public_upload", target_id=saved_as, detail=f"{file_name} held in sandbox as {saved_as}; verdict={verdict}; risk={report.get('risk_level')}; event_id={event_id}")
        db.commit()
        return {"ok": True, "filename": file_name, "saved_as": saved_as, "size": len(content), "url": None, "upload_state": "quarantined", "quarantined": True, "stored_in": "quarantine", "audit": report["audit"], "security_alert": {"style": "ids_quarantine", "http_status_hint": 403, "title": "上传审计未通过", "message": audit.get("summary"), "detail": f"样本已扣留到安全沙箱，编号 {saved_as}。"}}
    _write_ids_log(db, current_user=None, action="ids_upload_release", target_type="public_upload", target_id=saved_as, detail=f"{file_name} released to accepted/{saved_as}; risk={audit.get('risk_level')}; mode={audit.get('analysis_mode')}")
    db.commit()
    return {"ok": True, "filename": file_name, "saved_as": saved_as, "size": len(content), "url": _accepted_url(request, saved_as), "upload_state": "accepted", "quarantined": False, "stored_in": "accepted", "audit": audit}

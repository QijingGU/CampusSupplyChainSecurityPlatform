/**
 * Demo Mock Server - 小而精，仅支撑一轮演示闭环
 * 展示：自然语言 → React推理 → 决策 → 执行 → 留痕 → 存入记忆
 */
const http = require('http')

const PORT = 5200

// 固定演示话术：仅匹配此类问题返回完整闭环
...
const DEMO_OTHER_RESPONSE = {
  code: 200,
  data: {
    reply: '当前为演示模式。请尝试输入：\n「现在什么物资可能短缺？需要补货吗？」\n以体验完整闭环：React 推理 → 决策建议 → 执行 → 留痕 → 存入记忆。',
    react: [],
    actions: [],
  },
}
...
server.listen(PORT, () => {
  ...
  console.log(`  - POST /api/ai/chat  演示提问：现在什么物资可能短缺？需要补货吗？`)
